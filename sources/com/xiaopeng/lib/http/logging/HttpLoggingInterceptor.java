...
public class HttpLoggingInterceptor implements Interceptor {
                ...
                if (logHeaders) {
                    ...
                    if (logBody && hasRequestBody) {
                        try {
                            ...
                            e = e;
                            pn.t(this.tag, "logForRequest error!", e);
    ...
    private Response logForResponse(Response response, long tookMs, LogCacher.ILog log) {
        ...
        boolean z = false;
        boolean logBody = level == level2;
        boolean logHeaders = (this.printLevel == level2 || this.printLevel == Level.HEADERS) ? true : true;
        try {
            try {
                code = clone.code();
                sb = new StringBuilder();
                ...
                if (logHeaders) {
                    ...
                    for (int i2 = 0; i2 < count; i2++) {
                        try {
                            ...
                            e = e;
                            pn.t(this.tag, "logForResponse error!", e);
                            log.log("<-- END HTTP");
                            return response;
                        } catch (Throwable th2) {
                            th = th2;
    ...
    private void bodyToString(Request request, LogCacher.ILog log) {
        try {
            ...
            pn.t(this.tag, "bodyToString error!", e);
        }
    }
}
