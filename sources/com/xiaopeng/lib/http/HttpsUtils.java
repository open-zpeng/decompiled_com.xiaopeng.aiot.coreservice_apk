...
public class HttpsUtils {
    ...
    public static OkHttpClient buildOkHttpClient(Context context) {
        ...
        loggingInterceptor.setPrintLevel((co.g() || Build.TYPE.equals("userdebug")) ? HttpLoggingInterceptor.Level.BODY : HttpLoggingInterceptor.Level.BASIC);
        ...
        synchronized (HttpsUtils.class) {
            ...
            if (needSecurity && !Security.isInitSuccess()) {
                ...
                if (!Security.isInitSuccessWithoutIndiv()) {
                    pn.s(TAG, "!Security.isInitSuccessWithoutIndiv, return false!");
                    return sIsInitialized;
                }
            }
            Module.register(ho.class, new ho(application));
            initOkHttpClient(application);
            if (SSLHelper.getX509TrustManager(application) == null) {
                pn.s(TAG, "KeyManager init fail, return false!");
                return sIsInitialized;
            }
            sIsInitialized = true;
            return true;
    ...
    private static void initOkHttpClient(Application application) {
        pn.a(TAG, "start to initOkHttpClient!!!");
        registerBroadcast(application);
        OkHttpClient okHttpClient = buildOkHttpClient(application);
        lk.h().k(application).l(okHttpClient);
        pn.a(TAG, "finish initOkHttpClient!!!");
    }

    private static void registerBroadcast(Context context) {
        IntentFilter filter = new IntentFilter();
        filter.addAction(ACTION_BROADCAST_CLIENT_SSL_UPDATE);
        filter.setPriority(Priority.UI_TOP);
    ...
    @Deprecated
    public static void setSSLSocketFactoryWrapper(ISSLSocketFactoryWrapper wrapper) {
    }

    public static void setSSLSocketWrapper(SSLHelper.ISSLSocketWrapper wrapper) {
        SSLHelper.setSSLSocketWrapper(wrapper);
    }

    public static void setHttpInitEventListener(kn listener) {
        ln.c(listener);
    }
}
