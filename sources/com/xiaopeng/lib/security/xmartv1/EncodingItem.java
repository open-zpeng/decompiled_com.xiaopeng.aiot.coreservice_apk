...
public final class EncodingItem {
    ...
    public static EncodingItem decode(byte[] encodedContent) {
        ...
        byte[] bArr = sMagicNumber;
        if (b != bArr[0] || byteBuffer.get() != bArr[1]) {
            pn.a(TAG, "Wrong magic number!");
            return sEmptyItem;
        } else if (byteBuffer.get() != sVersion) {
            pn.a(TAG, "Wrong version number!");
            return sEmptyItem;
        } else {
            EncodingItem item = new EncodingItem();
            ...
            if (output != null && output.length > 0) {
                ...
                byte[] bArr3 = new byte[rawContentLength];
                item.mRawContent = bArr3;
                decodedBuffer.get(bArr3);
                return item;
            }
            return sEmptyItem;
        }
    ...
    public String rawContentString() {
        ...
        try {
            String result = new String(this.mRawContent, XmartV1Constants.UTF8_ENCODING);
            return result;
        } catch (UnsupportedEncodingException e) {
            pn.a(TAG, "Failed to convert raw content to string!");
            return "";
        }
    }
    ...
    private boolean encodeInternal() {
        ...
        if (output == null || output.length == 0) {
            pn.a(TAG, "Failed to encrypt the string.");
            return false;
        }
