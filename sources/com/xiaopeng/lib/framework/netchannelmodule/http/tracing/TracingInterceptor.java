...
public class TracingInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (!isXiaopengHost(url.host())) {
            pn.a(TAG, "isXiaopengHost(url.host()) == false, host:" + url.host());
        ...
        pn.a(TAG, "Tracing intercept operationName:" + operationName + " traceId:" + traceId + " spanId:" + spanId + " sampled:" + sampled);
        ...
        try {
            ...
            pn.a(TAG, "Tracing intercept finish, code:" + responseCode);
            return response;
        } catch (Exception e2) {
            e = e2;
            Exception exc = e;
            ...
            pn.a(TAG, "Tracing intercept finish, code:-1");
            throw e;
        }
    }
    ...
    private void decorateAndFinishSpan(Span span, String operationName, String method, HttpUrl url, int responseCode, Exception error) {
        span.name(operationName).kind(Span.Kind.CLIENT).tag(TAG_KEY_HOST, url.host()).tag(TAG_KEY_METHOD, method).tag(TAG_KEY_PATH, url.encodedPath()).tag(TAG_KEY_URL, url.toString()).tag(TAG_KEY_STATUS, String.valueOf(responseCode)).tag(TAG_KEY_VID, String.valueOf(un.g())).tag(TAG_KEY_VIN, un.f()).tag(TAG_KEY_CDUID, un.c()).tag(TAG_KEY_SID, un.e()).tag(TAG_KEY_NET, String.valueOf(rn.a(GlobalConfig.getApplicationContext()))).error(error).finish();
    }
}
