...
public abstract class BaseHttpCounter implements Handler.Callback {
    ...
    public static String sKeyFailed = "net_http_failed";
    public static String sKeySucceed = "net_http_succeed";
    public static String sKeyRequest = "net_http_request";
    public static String sKeyTrafficRx = "net_http_rx";
    public static String sKeyTrafficTx = "net_http_tx";
    public static String sKeyDetails = "net_http_details";
    public static String sKeyLastDate = "net_http_date";

    private BaseHttpCounter() {
        this.mEnabled = false;
        this.MIN_SAVING_INTERVAL = 30000;
        this.mStatEventName = "";
        this.mMoleButtonId = "";
        this.mHandler = null;
    }

    public BaseHttpCounter(String eventName, String buttonId) {
        this.mEnabled = false;
        this.MIN_SAVING_INTERVAL = 30000;
    ...
    public synchronized void init() {
        ...
        if (z) {
            ...
            try {
                ...
                pn.f(TAG, "Failure to load from preference due to " + ex);
            }
            debugInfo();
        }
    }
    ...
    public synchronized void increaseRequest(String domain, long sent) {
        if (this.mEnabled) {
            ensureInitialized();
            long j = this.mRequestCount + 1;
            this.mRequestCount = j;
            this.mEditor.putLong(sKeyRequest, j);
    ...
    public synchronized void increaseFailureWithSize(String domain, long received) {
        TrafficData data;
        if (this.mEnabled) {
    ...
    public synchronized void increaseSucceedWithSize(String domain, long received) {
        TrafficData data;
        if (this.mEnabled) {
    ...
    public synchronized void addReceivedSize(String domain, long received) {
        TrafficData data;
        if (this.mEnabled) {
    ...
    public synchronized void addSentSize(String domain, long sent) {
        TrafficData data;
        if (this.mEnabled) {
            this.mTotalData.addSentSize(sent);
    ...
    private synchronized void commitData() {
        ...
        if (this.mTotalData.receivedSize() >= MAX_VALUE_THRESHOLD || this.mTotalData.sentSize() >= MAX_VALUE_THRESHOLD || this.mLastDate < currentDate) {
            ...
            this.mEditor.putLong(sKeyLastDate, currentDate);
            clearCounters();
        }
    }
    ...
    private HashMap<String, TrafficData> initFromPreference(SharedPreferences preferences) {
