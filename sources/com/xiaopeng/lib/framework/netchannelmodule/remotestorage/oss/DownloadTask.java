...
public class DownloadTask extends BaseOssTask {
        ...
        oss.asyncGetObject(new GetObjectRequest(bucketRootName(), this.mRemoteObjectKey), new OSSCompletedCallback<GetObjectRequest, GetObjectResult>() { // from class: com.xiaopeng.lib.framework.netchannelmodule.remotestorage.oss.DownloadTask.1
            ...
            public void onSuccess(GetObjectRequest request, GetObjectResult result) {
                ...
                try {
                    ...
                    pn.f("NetChannel-DownloadTask", reason);
                }
                if (errorCode == 0) {
                    pn.j("Succeed to download file to :" + DownloadTask.this.mLocalFilePath + ", size:" + DownloadTask.this.mLocalFileSize);
                    DownloadTask.this.doSuccess();
                    StorageCounter.getInstance().increaseSucceedWithSize(DownloadTask.this.mLocalFileSize);
                    return;
                }
                on.b(DownloadTask.this.mLocalFilePath);
    ...
    public int writeContentToFile(GetObjectResult result) {
        ...
        try {
            try {
                ...
                pn.f("NetChannel-DownloadTask", "Failed to download file:" + exception);
            }
            if (result.getContentLength() != -1 && this.mLocalFileSize != result.getContentLength()) {
                pn.a("NetChannel-DownloadTask", "Not download completed. Expected:" + result.getContentLength() + ", but actual:" + this.mLocalFileSize);
    ...
    private void closeStream(Closeable stream) {
        if (stream != null) {
            try {
                ...
                pn.f("NetChannel-DownloadTask", "Failed to close stream: " + e);
            }
        }
    }
    ...
    public class FutureTaskCallback implements TokenRetriever.IRetrievingCallback {
        ...
        public void onSuccess(Token token) {
            ...
            wn.i(new Runnable() { // from class: com.xiaopeng.lib.framework.netchannelmodule.remotestorage.oss.DownloadTask.FutureTaskCallback.1
