...
public class SimpleAwsUploadTask extends BaseAwsTask {
    ...
    private void UpdateUploadInfo(UploadInfo uploadInfo) {
        ...
        if (map == null || map.isEmpty() || this.mCallbackParams.get(KEY_CALLBACK_BODY) == null || !XMART_OSSFILE_URL.equals(this.mCallbackParams.get(KEY_CALLBACK_URL))) {
            return;
        }
        try {
            ...
            String str = uploadInfo.file;
            if (str != null && str.startsWith("fra-xp-log/")) {
    ...
    private JsonObject wrapJsonBody(UploadInfo uploadInfo) {
        ...
        jsonObject.addProperty(AccountConfig.KEY_APP_ID, uploadInfo.app_id);
        ...
        jsonObject.addProperty(AccountConfig.KEY_TIMESTAMP, uploadInfo.timestamp);
        ...
        http.bizHelper().post(PRE_SIGN_URL, jsonObject.toString()).build().execute(new Callback() { // from class: com.xiaopeng.lib.framework.netchannelmodule.remotestorage.aws.SimpleAwsUploadTask.1
            ...
            public void onSuccess(IResponse iResponse) {
                try {
                    ...
                    String url = resObj.getString(Progress.URL);
    ...
    private String createSign(String file, String timestamp) {
        String appString = "file" + file + "type" + UploadInfo.UPLOAD_TYPE + "v" + UploadInfo.VERSION;
        String sign = EncryptUtil.MD5("XP-Appid" + timestamp + appString + "B638C588DCAD7C1A43E6FB").toLowerCase();
        return sign;
    }
    ...
    public void executeUploadTask(final String url) {
        wn.i(new Runnable() { // from class: com.xiaopeng.lib.framework.netchannelmodule.remotestorage.aws.SimpleAwsUploadTask.2
    ...
    public void upload2AwsByNormal(String urlStr) {
        File file = new File(this.mLocalFilePath);
        MediaType mediaType = MediaType.parse(OSSConstants.DEFAULT_OBJECT_CONTENT_TYPE);
        ...
        try {
            ...
            pn.g(TAG, "upload error!", e);
            doFailure(new StorageExceptionImpl(StorageException.REASON_UPLOAD_ERROR, e.getMessage()));
        }
    }
